...
for (jj in 1:nsim){
  for (m in 1:(l/q)){
    chi.sq.N.emp.low[jj,m]=qchisq(0.05/2,df=k)*V.inverse*N.emp[jj,m]  ## upper confidence interval for N.emp at alpha=0.05
    
    chi.sq.N.emp.up[jj,m]=qchisq(0.05/2,df=k,lower.tail=FALSE)*V.inverse*N.emp[jj,m] ## lower confidence interval for N.emp at alpha=0.05
